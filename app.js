//HAYKAP : This parameter is used for loading HOST with PHP
// var Host = "localhost";
...
function setOverlay(htmlClass, htmlElement, onClickFunction) {
	...
	if (!videoPlayOverlay) {
		...
		// HAYKAP : This removed de GREY SCREEN, but does not autoplay stream
...
function fTestButton(){
	console.log("TestButton Pressed")
	
	let descriptor ={
		TESTBUTTON: "casa azul"
	}
	emitUIInteraction(descriptor);
}
...
function showConnectOverlay() {
	...
	// HAYKAP Unhide this to get rid of the "Click to start" or better change var value o top
	// connect();
...
// HAYKAP: Gets rid of the need for an extra PLAY
...
function setupWebRtcPlayer(htmlElement, config) {
	...
	webRtcPlayerObj.onDataChannelConnected = function () {
		if (ws && ws.readyState === WS_OPEN_STATE) {
			// HAYKAP : Changed the message
			// showTextOverlay('WebRTC connected, just waiting for video');
			showTextOverlay('Please click to start the experience');
		}
	};
...
//HAYKAP Controls mouse lock, allow smooth transition between HTML and streaming frame
...
function resizePlayerStyle(event) {
	...
	//HAYKAP: CheckBox is not RENDERING on HTML, this moves the frame to somewhere esle in the midlle, so checkbox is null!
	// if (checkBox !== null) {
	if (checkBox == null) {
...
function connect() {
	...
	//HAYKAP : Comment this following line when working with PHP
	ws = new WebSocket(window.location.href.replace('http://', 'ws://').replace('https://', 'wss://'));
	
	//HAYKAP : This following line works for loading the IP from the URL
...
//HAYKAP : Use this instead when loading IP in the URL and the index is PHP
// function load(HostR) {
// 	Host = HostR;
// 	setupHtmlEvents();
// 	setupFreezeFrameOverlay();
