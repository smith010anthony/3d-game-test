...
function onJumpButton(command, item) {
...
function onUICommand(command, value) {
...
function myHandleResponseFunction(data) {
	...
	if(data) {
	console.log("MY HANDLED RESPONSE IS GETTING CALLED ALL RIGHT");
	}

	if(!data) {
		console.log("NO DATA - NO DATA");
	}

	if (data == 'MakeGREEN') {
		console.log("GREEN GRENN GREEN");
		// change bg color to green.
		document.getElementById('sidebar').style.background="green";
		console.log("GREN GRENN GRENNG GREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEN");
	}
	...
	//HAYKAP - Tobe included in myHandleResponseFunction up here. temporaly hidden - no need to handle resoutions for MVP
...
function onInteract() {
	...
	console.log("ONT INTERACT IS GETTING CALLED ALL RIGHT");
...
function changeControl( item, index) {
	item.classList.toggle("is-disabled");
	switch (index) {
		case 1:
			if (item.classList.contains('is-disabled')) {
				item.setAttribute('aria-label', 'Turn on');
			} else {
				item.setAttribute('aria-label', 'Turn off');
			}
			break;
		case 2:
			if (item.classList.contains('is-disabled')) {
				item.setAttribute('aria-label', 'With sound');
			} else {
				item.setAttribute('aria-label', 'Without sound');
			}
			break;
		case 3:
			if (item.classList.contains('is-disabled')) {
				item.setAttribute('aria-label', 'First person perspective');
			} else {
				item.setAttribute('aria-label', 'Third person perspective');
			}
			break;
		case 4:
			if (item.classList.contains('is-disabled')) {
				item.setAttribute('aria-label', 'Auto mode');
			} else {
				item.setAttribute('aria-label', 'Game mode');
			}
	}
}
...
function menuActive(item) {
	...
	var z = document.getElementsByClassName("classroom");
	z[0].classList.remove("is-active");
}

function openMenu() {
	var z = document.getElementsByClassName("classroom");
	z[0].classList.add("is-active");
}